...
CREATE POLICY "Public Access" ON storage.objects FOR SELECT USING (bucket_id IN ('profile-pictures', 'Edu_cert', 'cv_resume', 'nysc_cert'));
CREATE POLICY "Authenticated Upload" ON storage.objects FOR INSERT TO authenticated WITH CHECK (bucket_id IN ('profile-pictures', 'Edu_cert', 'cv_resume', 'nysc_cert', 'interview-videos'));
