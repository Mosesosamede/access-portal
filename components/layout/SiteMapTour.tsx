...
export default function SiteMapTour() {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="fixed inset-0 z-[100] bg-black/50 flex items-center justify-center p-4">
      <div className="bg-[#112240] p-8 rounded-3xl border border-white/10 max-w-sm">
        <h2 className="text-2xl font-bold mb-4 text-[#DFFF00]">Welcome!</h2>
        ...
        <button onClick={() => { localStorage.setItem('tourTaken', 'true'); setShow(false); }} className="w-full bg-[#d9f0dd] text-[#0A192F] p-3 rounded-xl font-bold">Got it!</button>
      </div>
    </motion.div>
  );
}
