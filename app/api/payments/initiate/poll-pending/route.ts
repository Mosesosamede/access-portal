...
async function runPendingPaymentPoll() {
  ...
  if (dbErr) {
    console.error('Error fetching pending purchases for polling:', dbErr);
    return NextResponse.json({ error: 'Database query error', details: dbErr.message }, { status: 500 });
  }
  ...
  for (const purchase of pendingPurchases) {
    ...
    try {
      ...
      if (isSuccessful && flwData) {
        console.log(`Polling found successful payment for txRef: ${txId}. Processing fulfillment...`);
      ...
      console.error(`Error checking tx ${txId}:`, pollErr);
      results.push({
        purchaseId: purchase.id,
        txId,
        status: 'error',
